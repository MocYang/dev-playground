...
class QuestionModalTests(TestCase):
    ...
    def test_was_published_recently_with_old_question(self):
        ...
        self.assertIs(old_question.was_published_recently(1), False)

    def test_was_published_recently_with_recent_question(self):
        time = timezone.now() - datetime.timedelta(hours=23, minutes=59, seconds=59)
        recenty_question = Question(pub_date=time)
        self.assertIs(recenty_question.was_published_recently(out_date_days=1), True)
...
class QuestionIndexViewTests(TestCase):
    ...
    def test_two_past_question(self):
        ...
        create_question(question_text='Past question2', days=-3)
        response = self.client.get(reverse('polls:index'))
