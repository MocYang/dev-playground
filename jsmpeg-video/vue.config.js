...
const configHTMLWebpackPlugin = (
  ...
  // console.log(Object.assign({}, config.options), doNeedStatic)
...
module.exports = {
  ...
  configureWebpack: config => {
    if (__PRODUCTION__) {
      ...
      let innerHTMLWebpackPluginOptions = null
      // 默认输出_assets.php
      config.plugins = config.plugins.map(plugin => {
        if (plugin.constructor === HtmlWebpackPlugin) {
          innerHTMLWebpackPluginOptions = Object.assign({}, plugin.options)
          plugin = configHTMLWebpackPlugin(plugin)
        }
        return plugin
      })

      //  yarn build:static，额外输出index.html
      if (__STATIC__ && innerHTMLWebpackPluginOptions) {
        config.plugins.push(configHTMLWebpackPlugin(
          new HtmlWebpackPlugin(),
          {
            template: innerHTMLWebpackPluginOptions.template,
            filename: innerHTMLWebpackPluginOptions.filename
          }, true))
      }
