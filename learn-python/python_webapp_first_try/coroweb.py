...
class RequestHandler(object):
    ...
    def __init__(self, app, fn):
        self._app = app
        self._func = fn
        self._has_request_arg = has_requested_arg(fn)
    ...
    async def __call__(self, request):
        ...
        if self._has_var_kw_arg or self._has_named_kw_arg or self._required_kw_arg:
...
def add_route(app, fn):
    ...
    if not asyncio.iscoroutinefunction(fn) and not inspect.isgeneratorfunction(fn):
        ...
        logging.info(
            'add route {method} {path} => {name}({param})'.format(
                method=method,
                path=path,
                name=fn.__name__,
                param=', '.join(inspect.signature(fn).parameters.keys())
            )
        )
...
def add_routes(app, module_name):
    ...
    if n_index == (-1):
        ...
        name = module_name[n_index + 1:]
        mod = getattr(__import__(module_name[:n_index], globals(), locals(), [name]), name)
